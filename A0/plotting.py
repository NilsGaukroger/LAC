...
df = pd.read_csv('../res/dtu10mw/rigid/DTU_10MW_rigid_hawc2s_u8000.ind',delim_whitespace=True,skiprows=1,names=columns)
...
power = pd.read_csv('../res/dtu10mw/rigid/DTU_10MW_rigid_hawc2s.pwr', delim_whitespace=True, skiprows=1, names=columns2)
data = []
for i in range(len(TSR)):
    data.append(pd.read_csv('../res/dtu10mw/rigid/DTU_10MW_rigid_hawc2s_u' + str(int(Uinf[i]*1000)) + '.ind', delim_whitespace=True, skiprows=1, names=columns))
...
if save == True:
    plt.savefig('figs/Part2a_subplots.pdf')
plt.show()
...
axs[2,1].set(xlabel='Spanwise position [m]')
if save == True:
    plt.savefig('figs/Part2b_subplots.pdf',bbox_inches='tight')
plt.show()

#%% Part 3: Chord, twist, thickness

columns3 = ['r', 'c', 't', 'set', 'opt']
blade = pd.read_csv('..\data\dtu10mw\DTU_10MW_RWT_ae.dat',delim_whitespace=True,skiprows=2,names=columns3)
...
axs[2].grid(True)
if save == True:
    plt.savefig('figs/Part3_subplots.pdf',bbox_inches='tight')
plt.show()

#%% Bonus level: Investigate flexibility

# Read data from .ind files
power2 = pd.read_csv('../res/dtu10mw/flex/DTU_10MW_flexible_hawc2s.pwr', delim_whitespace=True, skiprows=1, names=columns2)
data2 = []
for i in range(len(TSR)):
    data2.append(pd.read_csv('../res/dtu10mw/flex/DTU_10MW_flexible_hawc2s_u' + str(int(Uinf[i]*1000)) + '.ind', delim_whitespace=True, skiprows=1, names=columns))
...
axs[2,1].set(xlabel='Spanwise position [m]')
if save == True:
    plt.savefig('figs/Bonus_subplots.pdf',bbox_inches='tight')
plt.show()
...
if save == True:
    plt.savefig('figs/Bonusa_subplots.pdf')
plt.show()