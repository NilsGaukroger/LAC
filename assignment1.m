...
[DTU.c,DTU.that,DTU.beta,DTU.t,DTU.r,DTU.C,DTU.BETA] = DTU10MW_des(1,dis);
...
x_test = [0.3, 0.35, 0.47, 0.5]; % cl,max - cl,des
[p1,p2] = desPolys(aerofoil,x_test,4);
...
c_max    = (rotor.R/DTU.R) * 6.2060; % scaled max chord of DTU 10 MW [m]
...
result.c(find(rotor.t./result.c < 0.241,1):end) =...
    rotor.t(find(rotor.t./result.c < 0.241,1):end) / 0.241;
...
plot(DTU.r/DTU.R,DTU.C,'x'); hold off
ylabel('Chord [m]');
...
plot(DTU.r/DTU.R,DTU.BETA,'x'); hold off
...
function [c,that,beta,t,x,C,BETA] = DTU10MW_des(plt,dis)
% Chord
