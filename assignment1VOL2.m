...
tsr     = 4:1:12; % tsr(s) [-] (NB: minimum 5)
%tsr     = 6.5; %6.61; % optimal tsr [-]
...
rotor.r = linspace(rotor.r_hub,rotor.R,475); %blade span in meters
...
splineY = [result.c(24) result.c(93)-0.31 result.c(195)];                            
xq = rotor.r(24:195);
...
x0 = lsqnonlin(@(x)residuals_induction(x,rotor,tsr(j),i,p,p1,p2,t_max,rotor.R,result.c(j,i)),x0,lb,ub,opts);
    ...
    result2.CP(j,1) = (2/rotor.R^2) * trapz(rotor.r(2:end),rotor.r(2:end).*result.cp(j,2:end));
%     result_tsr.CT(j,1) % can't remember the equation for this right now
end
...
figure
plot(rotor.r/rotor.R,result2.a)
...
if length(tsr) ~= 1
    figure
    plot(tsr,result2.CP) %_____changed result to result2
    xlabel('TSR [-]'); ylabel('C_P [-]')
    grid on
    % Optimal TSR
    [CPmax,CPmax_idx] = max(result2.CP); %_____changed result to result2
...
HAWC.beta = interp1(rotor.r,result2.beta,(HAWC.r+rotor.r_hub));
