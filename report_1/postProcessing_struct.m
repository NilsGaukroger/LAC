...
for i = 1:2
    for j = 1:length(elas)
        for k = 1:size(vars,1)
            ...
            elseif i == 2
                rotor.(elas{j}).(vars{k,1}) =...
                    rotor.(elas{j}).(vars{k,2}) .* rotor.(elas{j}).(vars{k,3});
            end
        end
        ...
        elseif i == 2
            rotor.(elas{j}).s_S = rotor.(elas{j}).r / rotor.bladeLength;
        end
    end
end
...
for i = 1:4
    ...
    plot(rotor.st_flex.s_S,rotor.st_flex.(vars{i,1})); hold off
